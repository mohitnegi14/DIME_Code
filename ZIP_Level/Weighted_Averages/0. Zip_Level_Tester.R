...
for(i in 1:5) {
  ...
  fwrite(contribs, glue('{cleaned}/test_zip_contribs_{cycles[i]}.csv'))
...
list_of_datasets_by_year <- list.files(path = glue('{cleaned}/'), pattern = 'test_zip_contribs_20*')
...
fwrite(full_df, glue('{output}/test_zip_contribs_combined.csv'))
...
full_df <- fread(glue('{output}/test_zip_contribs_combined.csv'))
...
for(i in 1:5) {
  ...
  fwrite(contribs, glue('{cleaned}/test_check_zip_contribs_{cycles[i]}.csv'))
  
}

list_of_datasets_by_year <- list.files(path = glue('{cleaned}/'), pattern = 'test_check_zip_contribs_20*')
